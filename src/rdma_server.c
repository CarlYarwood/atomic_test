...
static struct ibv_mr *client_metadata_mr = NULL, *lock_mr = NULL, *server_metadata_mr = NULL;
...
static int accept_client_connection()
{
	 ...
	 * yet because we have to do lots of resource pre-allocation */
       ...
       conn_param.responder_resources = 3; /* For this exercis , we put a sma   number */
       ret = rdma_accept(cm_client_id, &conn_param);
       if (ret) {
	       rdma_error("Failed to accept the connection, errno: %d \n", -errno);
	       return -errno;
       }
       /* We expect an RDMA_CM_EVNET_ESTABLISHE  to indicate that the RDMA  
...
static int send_server_metadata_to_client() 
{
	...
	* do RDMA READs and WRITEs. */
       lock_mr = rdma_buffer_register(pd /* which protection domain */,
           lock,
           sizeof(lock),
	...
	*/
       ...
       server_metadata_attr.stag.local_stag = (uint32_t) lock_mr->lkey;
