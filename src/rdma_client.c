...
static int client_xchange_metadata_with_server()
{
	...
	client_metadata_attr.address = (uint64_t) NULL; 
	client_metadata_attr.length = 0; 
	client_metadata_attr.stag.local_stag = (uint32_t)NULL;
...
static int client_remote_memory_ops() 
{
	struct ibv_wc wc;
	int ret = -1;
  client_node_buffer_mr = rdma_buffer_alloc(pd,
      sizeof(node_id),
      (IBV_ACCESS_LOCAL_WRITE));
	...
	/* now we fill up SGE */
	client_send_sge.addr = (uint64_t) client_node_buffer_mr->addr;
	client_send_sge.length = (uint32_t) client_node_buffer_mr->length;
	client_send_sge.lkey = client_node_mr->lkey;
	/* now we link to the send work request */
	bzero(&client_send_wr, sizeof(client_send_wr));
	client_send_wr.sg_list = &client_send_sge;
	client_send_wr.num_sge = 1;
	client_send_wr.opcode = IBV_WR_ATOMIC_CMP_AND_SWAP;
  client_send_wr.atomic.rkey = server_metadata_attr.stag.remote_stag;
  client_send_wr.atomic.remote_addr = server_metadata_attr.address;
  client_send_wr.atomic.compare_add = 0;
  client_send_wr.atomic.swap = *node_id; 
	client_send_wr.send_flags = IBV_SEND_SIGNALED;
	/* Now we post it */
	ret = ibv_post_send(client_qp, 
		       &client_send_wr,
